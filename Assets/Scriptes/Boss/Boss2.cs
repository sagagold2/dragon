...
public enum BossState2 { MoveToAppearPoint = 0, Phase01, Phase02, Phase03 }
...
public class Boss2 : MonoBehaviour
{
    ...
    private BossState2 bossState2 = BossState2.MoveToAppearPoint;
    ...
    public void ChangeState(BossState2 newState)
    {
        StopCoroutine(bossState2.ToString());

        bossState2 = newState;

        StartCoroutine(bossState2.ToString());
    ...
    private IEnumerator MoveToAppearPoint()
    {
        ...
        while (true)
        {
            if (transform.position.z <= bossAppearPoint)
            {
                ...
                //Phase01 상태로 변경
                ChangeState(BossState2.Phase01);
    ...
    private IEnumerator Phase01() // 제자리에서 원 방사형 발사
    {
        ...
        while (true)
        {
            if (bossHP.CurrentHP <= bossHP.MaxHP * 0.7f) //보스 체력이 70%이하가 되면
            {
                ...
                ChangeState(BossState2.Phase02); //보스 페이즈 2로 변경
    ...
    private IEnumerator Phase02() //상하 이동하면 단일발사
    {
        ...
        while (true)
        {
            ...
            if (bossHP.CurrentHP <= bossHP.MaxHP * 0.3f) 
            {
                ...
                //Phase03 으로 변경
                ChangeState(BossState2.Phase03);
